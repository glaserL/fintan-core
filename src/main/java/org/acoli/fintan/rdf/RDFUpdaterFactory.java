...
//		}
		...
		if (conf.get("sentenceTimeout") != null) {
			int sentenceTimeout = conf.get("sentenceTimeout").asInt();
			if (sentenceTimeout > 0)
				LOG.info("Setting sentence timout to "+sentenceTimeout);
				updater.setSentenceTimeout(sentenceTimeout);
		}
